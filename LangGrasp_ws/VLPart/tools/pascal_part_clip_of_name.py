...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_path', default='datasets/metadata/pascal_part_clip_of+cname.npy')
    ...
    cat_names = [x['name'].lower().strip() for x in \
                 sorted(PASCAL_PART_CATEGORIES, key=lambda x: x['id'])]
    if args.use_part:
        cat_names = [x.split(":")[1] + ' of a ' + x.split(":")[0] for x in cat_names]
