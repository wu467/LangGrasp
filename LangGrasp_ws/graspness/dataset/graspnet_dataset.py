...
import graspness.utils as utils

# 数据处理类,继承自父类 Dataset，加载数据集中的数据

class GraspNetDataset(Dataset):
    def __init__(self, root, grasp_labels=None, camera='kinect', split='train', num_points=20000, voxel_size=0.005, remove_outlier=True, augment=False, load_label=True):
    ...
    def augment_data(self, point_clouds, object_poses_list):
        ...
        if np.random.random() > 0.5:
            flip_mat = np.array([[-1, 0, 0],
                                 [0, 1, 0],
                                 [0, 0, 1]])
            point_clouds = utils.transform_point_cloud(point_clouds, flip_mat, '3x3')
        ...
        rot_mat = np.array([[1, 0, 0],
                            [0, c, -s],
                            [0, s, c]])
        point_clouds = utils.transform_point_cloud(point_clouds, rot_mat, '3x3')
        for i in range(len(object_poses_list)):
    ...
    def get_data(self, index, return_raw_cloud=False):
        ...
        try:
            ...
            print(repr(e))
            print(scene)
        camera = utils.CameraInfo(1280.0, 720.0, intrinsic[0][0], intrinsic[1][1], intrinsic[0][2], intrinsic[1][2],
                            factor_depth)

        # generate cloud
        cloud = utils.create_point_cloud_from_depth_image(depth, camera, organized=True)
        ...
        if self.remove_outlier:
            ...
            workspace_mask = utils.get_workspace_mask(cloud, seg, trans=trans, organized=True, outlier=0.02)
